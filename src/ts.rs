...
///   ));
/// ```
pub fn ts_prelude<W: Write>(buf: &mut W, now: String, bin_name: &str) {
    ln!(
        buf,
...
/// ```
pub fn ts_extend_traits<W: Write>(buf: &mut W, contract: &Contract) {
...
/// "#);
/// ```
pub fn ts_contract_methods<W: Write>(buf: &mut W, contract: &Contract) {
    ...
    ln!(buf, "export const {}Methods = {{", contract.name);
    ln!(
        buf,
        "    viewMethods: [\n{}    ],",
...
/// "#);
/// ```
pub fn ts_items<W: Write>(buf: &mut W, items: &Vec<Item>, contract: &Contract) {
    for item in items {
        match item {
            Item::Mod(item_mod) => {
...
/// "#);
/// ```
pub fn ts_impl<W: Write>(buf: &mut W, item_impl: &ItemImpl, contract: &Contract) {
...
/// assert_eq!(String::from_utf8_lossy(&buf), "");
/// ```
pub fn ts_struct<W: Write>(buf: &mut W, item_struct: &ItemStruct) {
    ...
    match &item_struct.fields {
        Fields::Named(fields) => {
            ln!(buf, "export type {} = {{", item_struct.ident);
...
/// "#);
/// ```
pub fn ts_enum<W: Write>(buf: &mut W, item_enum: &ItemEnum) {
    if !item_enum.is_serde() {
        return;
    }

    ts_doc(buf, &item_enum.attrs, "");
    ln!(buf, "export enum {} {{", item_enum.ident);
...
/// "#);
/// ```
pub fn ts_typedef<W: Write>(buf: &mut W, item_type: &syn::ItemType) {
...
///
pub fn ts_doc<W: Write>(buf: &mut W, attrs: &Vec<Attribute>, indent: &str) {
    ln!(buf, "{}/**", indent);
